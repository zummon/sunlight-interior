...
<div class="uk-grid-collapse" uk-grid="masonry: true">
  {#each data.services as { image, path, title, description, date }, index (`service-${index}`)}
    <div class="uk-width-1-1 uk-width-1-2@s">
      <div class="uk-cover-container uk-box-shadow-medium" style="min-height: 80vh">
        <img {...image} uk-cover="" />
...
<div class="uk-card uk-card-body uk-card-default uk-margin-auto-left uk-margin-auto-right" style="width: 50vw;">
  <h2 class="uk-heading-divider">Total Price</h2>  
  <p class="uk-margin-remove-vertical uk-text-lead">Labor: {data.services.reduce((prev,curr) => (prev+curr.price.labor),0)}</p>
  <p class="uk-margin-remove-vertical uk-text-lead">Material: {data.services.reduce((prev,curr) => (prev+curr.price.material),0)}</p>
</div>