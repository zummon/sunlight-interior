...
<div class="uk-child-width-1-2@s uk-grid-collapse" uk-grid="masonry: true" uk-lightbox="animation: slide">
  {#each [image, ...images].filter(img => img.src) as image, index (`image-${index}`)}
    <div>
      <a class="uk-display-block uk-box-shadow-medium" href={image.src} data-caption={image.alt}>
        <img {...image}/>
      </a>
    </div>
  {/each}
...
<a class="uk-display-block uk-card uk-card-body uk-card-default uk-link-toggle uk-margin-auto-left uk-margin-auto-right" href="/#contact" style="width: 50vw;">
  <h2 class="uk-heading-divider">Price</h2>  
  <p class="uk-margin-remove-vertical uk-text-lead">Labor: {price.labor}</p>
  <p class="uk-margin-remove-vertical uk-text-lead">Material: {price.material}</p>
